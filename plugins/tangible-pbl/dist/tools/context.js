...
import { switchEnvironment } from '../server.js';
import { text, banner } from './render.js';
export const registerContextTools = (server, rt) => {
    server.tool('pbl_whoami', 'Show the active Tangible environment, business and role.', {}, async () => {
    ...
    server.tool('pbl_list_businesses', 'List the businesses this account can author courses for.', {}, async () => {
        const current = rt.current;
    ...
    server.tool('pbl_use_business', 'Log in to a business by name. This is gate 1 — it confirms which company you are authoring into.', { name: z.string().describe('Business name, or a unique prefix of it') }, async ({ name }) => {
        const current = rt.current;
    ...
    server.tool('pbl_use_environment', 'Switch between staging and production. Clears the login; refuses while a session is open.', { env: z.enum(['staging', 'production']) }, async ({ env }) => {
