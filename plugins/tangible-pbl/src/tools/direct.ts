...
export const registerDirectTools = (
  server: McpServer,
  rt: { current: Runtime },
): void => {
  server.tool(
    'pbl_open_in_app',
    'Return the Tangible app URL for a course, for eyes-on review.',
    { courseId: z.string() },
    async ({ courseId }) => {
  ...
  server.tool(
    'pbl_add_resource',
    'Attach a link or text resource to a sub-content unit.',
    {
      courseId: z.string(),
    ...
    async ({ courseId, contentUnitId, subUnitId, ...values }) => {
      const current = rt.current;
      await addResource(
        current.http, current.auth, courseId, contentUnitId, subUnitId, values,
      );
